...
configured_hosts = [
    ...
    for host in config(
        "ALLOWED_HOSTS",
        default=f"127.0.0.1,localhost,{render_hostname},shopzone-e-commerce.onrender.com",
    ).split(",")
    if host.strip()
]
...
def _database_config_from_url(database_url: str) -> dict:
    ...
    if engine == "django.db.backends.mysql":
        ...
        mysql_ssl_ca = config("MYSQL_SSL_CA", default="/etc/secrets/aiven-ca.pem").strip()
        if mysql_ssl_ca:
            db_config["OPTIONS"]["ssl"] = {"ca": mysql_ssl_ca}
